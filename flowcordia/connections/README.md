...
| Flowcordia operations worker | Durable operations heartbeat | Let request-serving replicas observe a dedicated worker's bounded liveness and timing budgets without sharing event credentials or worker identity | Singleton expiring heartbeat implemented; queue and proposal health remain independent checks |
