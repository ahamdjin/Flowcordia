...
| Release-candidate evidence gate | Exact live dependency, backup, restore, and upgrade evidence chain | One bounded schema `0.1` validator and CLI bind release identity, current/target revisions, migration counts, recovery digests, READY checks, chronology, freshness, and the connected-acceptance rollout step before destructive release work; the underlying probes and connected release remain separate |
...
| Self-hosting | Guided setup over existing Docker/Kubernetes topology | Deterministic configuration, live-dependency, database recovery, controlled-upgrade, release-candidate, provider-readiness, and alert-readiness gates plus install ordering delivered; automated mutation, HA, PITR, and DR remain planned |
