...
function environment(overrides: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...
    CLICKHOUSE_URL: "http://default:strong-clickhouse-password@clickhouse:8123/default?secure=false",
...
describe("Flowcordia bundled self-host topology", () => {
  ...
  it("allows private HTTP only for the exact MinIO and S2 service identities", () => {
    ...
    expect(projection({ REALTIME_STREAMS_S2_ENDPOINT: "http://s2:8080/v1" }).state).toBe(
      "BLOCKED"
    );
