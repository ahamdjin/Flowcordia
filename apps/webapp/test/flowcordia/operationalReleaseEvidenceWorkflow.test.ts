...
describe("Flowcordia operational release evidence workflows", () => {
  ...
  it("requires all ten official run identities in the immutable assembler", () => {
    const source = workflow(".github/workflows/flowcordia-assemble-release-evidence.yml");
    expect(source).toContain("source_runs_json:");
    expect(source).toContain(
      "Exact JSON object with bundled_clean_install, self_host_lifecycle, provider, alert, preview, promotion, production, webhook_production, rollback_proposal, and rollback_production run IDs"
    );
    for (const stage of [
    ...
    expect(source).toContain("FLOWCORDIA_RELEASE_BUNDLED_CLEAN_INSTALL_RUN_ID");
    ...
    expect(source).toContain(".github/workflows/flowcordia-bundled-clean-install.yml");
