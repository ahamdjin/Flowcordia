...
describe("Flowcordia webhook-bound launch manifest", () => {
  ...
  it("requires exactly one production webhook artifact", () => {
    expect(() => assemble(launchEvidenceSources().filter((source) => source.stage !== "webhook_production"))).toThrow(
      FlowcordiaWebhookReleaseEvidenceError
    );
  ...
  it("rejects sensitive endpoint identity in webhook evidence", () => {
    ...
    const sources = launchEvidenceSources().filter((source) => source.stage !== "webhook_production");
  ...
  it("requires the successor to be exactly the next generation", () => {
    ...
    const sources = launchEvidenceSources().filter((source) => source.stage !== "webhook_production");
    expect(() => assemble([...sources, webhookReleaseSource(evidence)])).toThrow(/next endpoint generation/);
  });
  ...
  it("requires the webhook workflow commit to equal the release application", () => {
    ...
    const sources = launchEvidenceSources().filter((candidate) => candidate.stage !== "webhook_production");
