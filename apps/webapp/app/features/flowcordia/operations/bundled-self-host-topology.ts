...
const SUPERVISOR_IMAGE =
  /^ghcr\.io\/triggerdotdev\/supervisor(?::[^\s]+|@sha256:[0-9a-f]{64})$/;
...
export function presentFlowcordiaBundledSelfHostTopology(input: {
  ...
  const checks: FlowcordiaBundledSelfHostTopologyCheck[] = [
    ...
    check(
      ...
      "MinIO is bound through the private S3-compatible object-store contract.",
      "The bundled MinIO object-store connection or credentials are incomplete or unsafe."
    ),
    check(
      "realtime_streaming",
