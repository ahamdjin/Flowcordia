...
import {
  FlowcordiaFunctionValidationTriggerError,
  triggerFlowcordiaFunctionValidation,
} from "./trigger.server";

const ValidationCommand = z
  .object({
...
export async function executeFlowcordiaFunctionValidationCommand(input: {
  ...
  const parsed = ValidationCommand.safeParse(body);
  if (!parsed.success) {
    return json(
      { ok: false, error: "invalid_request", message: "Unsupported validation command." },
      400
    );
  }

  const taskIdentifier = `flowcordia-validate-${parsed.data.workflowId}`;
  if (!input.ability.can("trigger", { type: "tasks", id: taskIdentifier })) {
    return json(
      {
        ok: false,
        error: "permission_denied",
        message: "You cannot validate this repository function suite.",
      },
      403
    );
  }
  ...
  try {
    ...
    const result = await triggerFlowcordiaFunctionValidation({
      scope,
    ...
    if (error instanceof FlowcordiaFunctionValidationTriggerError) {
      return json(
        {
          ok: false,
          error: error.code,
    ...
    return json(
      {
        ...
        message: "Repository function validation could not be started.",
        retryable: true,
      },
      503
    );
