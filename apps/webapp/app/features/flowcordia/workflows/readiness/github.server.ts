...
function permissionState(
  ...
  return permissionRank(permissions[permission]) >= permissionRank(minimum)
    ? "PASSED"
    : "BLOCKED";
}
...
async function inspectInstallation(
  ...
  try {
    ...
    const state =
      status === 401 || status === 403 || status === 404 ? "BLOCKED" : "UNAVAILABLE";
    return {
      installation: state,
      contents: state,
      pullRequests: state,
...
export async function createFlowcordiaReadinessGitHub(scope: WorkflowIndexScope) {
  ...
  return {
    ...
    async readTextFile(path: string, commitSha: string): Promise<FlowcordiaReadinessTextFile> {
      ...
      if (
        result.size > MAX_TEXT_FILE_BYTES ||
        (result.size > 0 && result.contentBase64 === "")
      ) {
