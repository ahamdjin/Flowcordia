...
function CheckIcon({ state }: { state: FlowcordiaRepositoryReadinessCheck["state"] }) {
  switch (state) {
    ...
    case "UNAVAILABLE":
      return <CircleDashedIcon className="mt-0.5 size-4 shrink-0 text-rose-300" />;
  }
}
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-3xl text-xxs leading-4 text-text-dimmed">
              Proves the server-owned repository binding, GitHub App installation permissions,
              exact production head, workflow index, Trigger.dev task discovery, and preview
              deployment setting. It never returns tokens, installation IDs, database IDs, or raw
              provider errors.
            </p>
          </div>
        </div>
