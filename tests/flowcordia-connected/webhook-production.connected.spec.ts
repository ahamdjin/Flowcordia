...
async function activateEndpoint(page: Page, card: Locator, nodeId: string, timeout: number) {
  ...
  if (!/^[A-Za-z0-9_-]{20,64}$/.test(publicId))
    throw new Error("Active endpoint identity is invalid.");
...
test("prove signed production webhook activation, incident revocation, and replacement", async ({
  ...
  try {
    ...
    await page
      .getByTestId("flowcordia-webhook-revocation-reason")
      .selectOption("manual_emergency_stop");
    await page
    ...
    if (
      replacementPublicId === original.publicId ||
      replacementGeneration !== original.generation + 1
    ) {
      throw new Error(
        "Replacement endpoint did not create the next immutable identity generation."
      );
    }
