...
function sourcePrefix(stage: FlowcordiaReleaseEvidenceStage): string {
...
): Promise<FlowcordiaReleaseEvidenceSource> {
...
export async function assembleFlowcordiaReleaseManifestFromEnvironment(
  environment: Record<string, string | undefined>
): Promise<FlowcordiaReleaseManifest> {
  ...
  const manifest = assembleFlowcordiaReleaseManifest({
    ...
    sources: await Promise.all(
      FLOWCORDIA_RELEASE_EVIDENCE_STAGES.map((stage) => source(environment, evidenceRoot, stage))
    ),
  });
