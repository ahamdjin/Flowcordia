...
const remove = /^(FLOWCORDIA_(?:CONFIG_FILE|SECRETS_FILE|RELEASE_MANIFEST_FILE|MIGRATION_STATE_DIR|DIAGNOSTICS_STATE_DIR|REGISTRY_AUTH_FILE|IMAGE_REFERENCE|IMAGE_DIGEST|APPLICATION_COMMIT_SHA|RELEASE_MANIFEST_SHA256|MIGRATION_CONFIRM|VOLUME_PREFIX|APPLICATION_NETWORK_NAME|SUPERVISOR_NETWORK_NAME|DOCKER_PROXY_NETWORK_NAME|STUDIO_BUILDER_DOCKER_PROXY_NETWORK_NAME|STUDIO_BUILD_NETWORK|HTTP_BIND|HTTP_PORT|MINIO_CONSOLE_PORT|REGISTRY_PORT|DEPLOY_REGISTRY_HOST)|DEPLOY_REGISTRY_HOST|APP_ORIGIN|LOGIN_ORIGIN|BROWSER_ORIGIN|EMAIL_TRANSPORT|FROM_EMAIL|REPLY_TO_EMAIL|GITHUB_APP_.+)\s*=/;
