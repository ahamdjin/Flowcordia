...
Generated files live under `.flowcordia/generated`, so repository-relative code paths are emitted as imports from that directory. Export names must be plain JavaScript identifiers; paths must be traversal-free. The proposal boundary additionally rejects code references that name a different repository.
