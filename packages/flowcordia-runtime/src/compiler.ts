  ...
  const source = [
    ...
    ...(validationTaskId
      ? [
          ...
          `    const writeMetadata = (status: "RUNNING" | "PASSED" | "FAILED", passedCount: number, failedCount: number, failureCode: string | null = null) => {`,
          `      metadata.set("flowcordiaValidation", {`,
          `        schemaVersion: "0.1",`,
          `        workflowId: workflow.id,`,
          `        proposalId: payload.proposalId,`,
          `        headSha: payload.headSha,`,
          `        suiteDigest: payload.suiteDigest,`,
          `        status,`,
          `        passedCount,`,
          `        failedCount,`,
          `        failureCode,`,
