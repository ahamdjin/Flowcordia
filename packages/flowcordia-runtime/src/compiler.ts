...
export function compileWorkflowToTriggerTask(
  ...
  const source = [
    ...
    ...(hasSubflowNodes
      ...
      : []),
    `  wait: async (configuration) => {`,
    `    if (configuration.mode === "until") {`,
    `      await wait.until({ date: new Date(configuration.untilTimestamp) });`,
    `      return;`,
    `    }`,
    `    await wait.for({ seconds: configuration.durationSeconds });`,
    `  },`,
    `  authorizeHttp: (url) => {`,
    ...
    `    const flowcordiaNodeTraces: Array<{ nodeId: string; operation: string; status: string; startedAt: string; completedAt: string; durationMs: number; message?: string }> = [];`,
    ...runtimePayload,
    `      onTrace: async (trace) => {`,
    ...
    `          traces: flowcordiaNodeTraces.slice(-100),`,
