...
export class GitHubRepositorySourcePatchStore {
  ...
  async save(
    ...
    if (!validated.success) {
      return inputError(operation, [...issues, ...validated.issues.map((issue) => issue.message)]);
    }
    if (issues.length > 0) return inputError(operation, issues);
    ...
    try {
      const snapshot = await this.#resolve(input.scope, patch.path, input.scope.repository.branch);
