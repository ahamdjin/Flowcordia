...
for entry in \
  "fbdbc8c6d27f7c57433f833734c4f9389dfab3ac4f2f1aa991c9bae5cc0bd3eb .agent/operations-cli-final.part0" \
  "5c7ff55f2da4f9319addcd6b8c1a7ff7b3ee1a22a76cf38239a56e86f20e7f71 .agent/operations-cli-final.part1" \
  "7fc99328de0ec26e5e3d132b75b249581fbc3e67958e3c61125dbd8eb47102e3 .agent/operations-cli-final.part2" \
  "1b15183f419e465f5f3d2aaf07c281080ae59a0a4b980034190c219d2a3dbdd5 .agent/operations-cli-final.part3"; do
  echo "$entry" | sha256sum --check
done

cat .agent/operations-cli-final.part{0,1,2,3} > /tmp/operations-cli-final.patch.gz.b64
echo "76f5ba5d1ef66f616f7a79d40e94d2f21d22258979386d124a08e4bc5371ab09 /tmp/operations-cli-final.patch.gz.b64" | sha256sum --check
base64 --decode /tmp/operations-cli-final.patch.gz.b64 | gunzip > /tmp/operations-cli-final.patch
echo "540ac8d617512b1de74a7d167eaafde6aebb4312e90d2d1bfce43ec3d70c2a13 /tmp/operations-cli-final.patch" | sha256sum --check
...
expect_not_usage image-evidence-valid pnpm exec tsx scripts/flowcordia-release-image-evidence.ts --manifest /tmp/flowcordia-release-manifest.json --repository owner/repository --run-id 123 --run-attempt 1 --attestation-id attestation-test --created-at 2026-07-27T00:00:00.000Z --output /tmp/flowcordia-image-evidence.json
...
expect_not_usage artifact-preflight-valid pnpm exec tsx scripts/flowcordia-self-host-artifact-preflight.ts --manifest /tmp/flowcordia-release-manifest.json --image-evidence /tmp/flowcordia-image-evidence.json --expected-repository owner/repository --expected-run-id 123 --expected-application-sha 0123456789abcdef0123456789abcdef01234567 --json
...
expect_not_usage clean-dependencies-valid pnpm exec tsx scripts/flowcordia-self-host-clean-dependencies.ts --manifest /tmp/flowcordia-release-manifest.json --output /tmp/flowcordia-clean-dependencies.json
...
expect_not_usage self-host-exec-valid pnpm exec tsx scripts/flowcordia-self-host-exec.ts --config /tmp/flowcordia.env --secrets /tmp/flowcordia.secrets --cwd /tmp -- node --version
...
expect_not_usage lifecycle-evidence-valid pnpm exec tsx scripts/flowcordia-self-host-lifecycle-evidence.ts --current-manifest /tmp/current-manifest.json --current-image-evidence /tmp/current-image.json --installation-identity-evidence /tmp/installation.json --clean-dependencies-evidence /tmp/clean.json --current-migration-evidence /tmp/current-migration.json --current-install-diagnostics /tmp/current-install.json --current-restart-diagnostics /tmp/current-restart.json --backup-manifest /tmp/backup.json --restore-evidence /tmp/restore.json --upgrade-evidence /tmp/upgrade.json --target-manifest /tmp/target-manifest.json --target-image-evidence /tmp/target-image.json --target-migration-evidence /tmp/target-migration.json --target-diagnostics /tmp/target-diagnostics.json --observations /tmp/observations.json --repository owner/repository --run-id 123 --run-attempt 1 --source-sha 0123456789abcdef0123456789abcdef01234567 --output /tmp/lifecycle-evidence.json
expect_usage self-host-validate-unknown pnpm exec tsx scripts/flowcordia-self-host-validate.ts --flowcordia-unknown-option
expect_not_usage self-host-validate-valid pnpm exec tsx scripts/flowcordia-self-host-validate.ts --config /tmp/flowcordia.env --secrets /tmp/flowcordia.secrets --manifest /tmp/flowcordia-release-manifest.json

node --input-type=module <<'NODE'
import assert from "node:assert/strict";
import { parseArguments } from "./scripts/flowcordia-upstream-drift.mjs";
assert.deepEqual(parseArguments(["--base", "origin/main", "--head", "HEAD", "--manifest", "ownership.json", "--json", "--fail-on-core"]), {
  base: "origin/main",
  head: "HEAD",
...
assert.throws(() => parseArguments(["--unknown"]), { message: "Unknown argument: --unknown" });
assert.throws(() => parseArguments(["unexpected"]), { message: "Unknown argument: unexpected" });
NODE
